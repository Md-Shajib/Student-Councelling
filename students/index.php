<?php
	...
	if(isset($_POST['submit'])){
		$ret=mysqli_query($bd, "SELECT * FROM students WHERE userEmail='".$_POST['username']."' and password='".md5($_POST['password'])."'");
		...
		if($num>0){
			...
			$status=1;
			$log=mysqli_query($bd, "insert into studentlog(uid,username,userip,status) values('".$_SESSION['id']."','".$_SESSION['login']."','$uip','$status')");
			...
			$uip=$_SERVER['REMOTE_ADDR'];
			$status=0;
			mysqli_query($bd, "insert into studentlog(username,userip,status) values('".$_SESSION['login']."','$uip','$status')");
	...
	$email=$_POST['email'];
		...
		$query=mysqli_query($bd, "SELECT * FROM students WHERE userEmail='$email' and contactNo='$contact'");
		$num=mysqli_fetch_array($query);
		if($num>0){
			mysqli_query($bd, "update students set password='$password' WHERE userEmail='$email' and contactNo='$contact' ");
